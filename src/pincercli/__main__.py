...
# Based Off discord.py's show_version script.
def show_version():
    entries = []
    ...
    entries.append(
        "- pincer v{0.major}.{0.minor}.{0.micro}-{0.releaselevel}".format(version_info)
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="pincercli", description="Pincer CLI Functions"
    )  # Doesn't effect you doing python -m pincercli insead of python -m pincer.
    parser.add_argument(
